...
function [frames, mov] = loadVideoFile(filename)
    ...
    frame_size = [450 1620]*1; % increase and standardize frame size
...
%     movout  = VideoWriter(fnameout, 'Uncompressed AVI' );    
%     movout.FrameRate  = d.movin.FrameRate;
    
%     open(movout)% open video file

    try delete([fnameout '.tif']), catch,end
    for i = 1:nframes
        frame = d.frames(:,:,:,i);
...
%         writeVideo(movout,uint8(imout))
        imwrite(uint8(imout), [fnameout '.tif'], 'writemode', 'append')
    end
    delete(lbl)
